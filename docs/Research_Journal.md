...
**Two secondary observations, both flagged as unconfirmed.** *Strafe is the weak axis*: at 0.05 m/s the `W`/`S` leg is clean while `D`/`A` fails on the same drive — one observation, mechanism unknown, and a candidate explanation involving `scan_relay`'s reflection was considered and not supported. *Speed matters*: at 0.10 m/s both legs fail with larger corrections; at 0.05 m/s only the lateral one does. Neither is a controlled result yet, and neither should be built on.
