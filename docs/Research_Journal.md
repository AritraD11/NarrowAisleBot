...
- **Two IMUs at diagonal extremes (e.g. near FR and RL) — considered and deferred, 5 Aug 2026.** With a known baseline (FR↔RL = 0.87 m on this chassis) two accelerometers can be differenced to solve for α and ω² independently of translation — the accelerometer-array / "gyro-free IMU" technique. Rejected as a *starting* configuration because: (a) it exists to replace a gyro, and the BNO055 already provides one in-chip; (b) near-wheel is the worst accelerometer location on the robot, closest to the motor/gearbox vibration that accelerometers pick up far more readily than gyros; (c) it doubles the encoder-era failure mode this project just spent two months on. **Genuinely worth revisiting as a redundancy upgrade:** both gyros must read identical ωz on a rigid body, so disagreement is an unambiguous fault signal, and BNO055's ADR pin (0x28/0x29) allows exactly two on one I²C bus with no multiplexer. `robot_localization` supports `imu0`/`imu1`.
