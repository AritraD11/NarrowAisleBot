...
| 08 Aug 2026  | v2.6        | Started the visualization phase (§17.5): added `foxglove_bridge` to `install.sh` and `aislebot_full.launch.py` (port 8765, always-on), with connection steps in `Important_Commands.md` §6 — not yet verified on hardware. Found and corrected a premise in `SLAM_Theory.md` §2.3 while scoping the follow-up: `slam_nodom.yaml`'s `odom_frame: odom` plus `odometry_publisher` running continuously via `aislebot.service` means every mapping run since §16.9's odom-TF fix has already had a live odometry prior, despite the filename — there's no second "with-odom" config to write. Revised the actual benchmark to a genuine no-prior control run (`odometry_publisher` stopped) vs. current-as-is, deferred until Foxglove Bridge is confirmed working so drift is visible directly rather than inferred from `unknownPct` alone. |
