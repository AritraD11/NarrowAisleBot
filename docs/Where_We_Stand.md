...
| 6 | **`map→odom`** (SLAM front end) | 🔴 **BROKEN, and now known to be route-dependent or intermittent** | ✅ | **One correction per pose-graph node, every node** (§17.44). §17.47: two legs on the *same config, same afternoon* gave **0.577 m vs 0.085 m** return-to-mark and **0.678 m vs 0.280 m** max correction — a 6.8× spread. The bad leg's 0.562 m/m cumulative is **worse than the 0.507 m/m pre-Stage-D baseline**. G2's 0.202 m / 0.163 m/m was a property of that drive, not of the robot |
...
| **Narrow furniture-flanked aisles are a degenerate geometry, like tight circles** | 🔷 | §17.47's bad leg threaded one twice in opposite directions; the good leg ran open floor. Same mechanism as §17.44 would predict it, but n=1 per route and **intermittency alone explains the spread just as well**. The repeat-each-leg test decides it |
