...
| 6 | **`map→odom`** (SLAM front end) | 🔴 **BROKEN, and confirmed intermittent, not route-dependent** | ✅ | §17.44: one correction per pose-graph node, every node. §17.47: two legs, same config/afternoon, 0.577 m vs 0.085 m return, 6.8× spread. §17.48's repeat test settles which: the **same front-leg aisle re-driven** returned **0.209 m** with a **0.857 m** max correction — the worst ever recorded, on a route that previously gave 0.577 m/0.678 m. Three outcomes on one route rules out geometry as the explanation. Back end confirmed healthy a fourth time (`graph_residuals`: 9 closures, `moved=0` throughout) |
| 7 | **Saved map** | 🔴 **Does not exist** | ✅ | Every map ever produced grades `FOLDED`. Best D2 doubled walls **1.9%** (gate <1.0%); 31 Aug legs 5.1% / 5.2%, unknown 73.8% / 79.4% (gate <50%). **One sub-criterion has now been met once** — return-to-mark **0.085 m** on `run_20260831_191509`, the first inside the <0.15 m gate — but never all four on one map. **There is still no accepted commissioning map** |
...
| **Narrow furniture-flanked aisles are a degenerate geometry, like tight circles** | ⛔ **RETRACTED as the primary explanation** | §17.48's repeat test: the same narrow aisle, re-driven, gave 0.209 m/0.857 m against the first attempt's 0.577 m/0.678 m — a route that is supposedly always-bad produced its best-ever return alongside its worst-ever peak. Geometry may still set the *ceiling* on how bad a correction can get; it does not explain *whether* one fires |
| **G2's pass generalises to other drives** | ⛔ **RETRACTED** | §17.47's front leg, on the identical deployed config, produced 0.678 m max correction (G2 gate: 0.30 m) and 0.562 m/m cumulative — worse than the pre-fix baseline. G2 was passed *on one route*; it is not a property of the robot |
