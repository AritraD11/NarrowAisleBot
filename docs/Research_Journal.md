...
# Part XVII — 6 August 2026: Ground Testing Begins, and a Second Diagonal-Check Bug

## 17.1 First ground runs, and `nab_pid_logger.py --mode rotate`

Three ground-floor drive sessions recorded the same day (`run_20260806_152810`, `run_20260806_183540`, `run_20260806_184938` — 15:28, 18:35, 18:49), robot on the floor at full weight, v3.0 firmware, gains still air-calibrated. All three: RMS tracking error 2.7–4.1% of peak, **0% PWM saturation**, **zero direction-sign faults**, in all three sessions, hours apart. The loop holds up under real load.

Floor space turned out to be a real constraint for the structured `plant`/`staircase` tests — both drive all four wheels equally, which on the ground means the robot travels in a straight line (`staircase` in particular is one continuous ramp with no reversal, potentially several metres by the top of its range). Added `--mode rotate` to `tools/nab_pid_logger.py`: applies the mecanum sign pattern for a pure yaw (`FR/RR` positive, `FL/RL` negative, matching `mecanumIK()`) instead of driving all four the same way, so the chassis spins roughly in place — same wheel-floor loading, far less floor needed. Not a mathematically perfect zero-drift spin (true zero drift needs the outer pair faster than the inner pair in velocity terms, and this is an open-loop PWM command, not velocity), but it turns a several-metre requirement into well under a metre. Fit output flags when a run was rotate-mode so the numbers aren't silently treated as identical to a straight-line measurement.

## 17.2 A second diagonal-check bug — same-sign wasn't a tight enough filter

`tools/analyze_bench_log.py`'s diagonal-pair check was fixed once already (Part XVI §16.2: exclude rotation, where a diagonal pair legitimately carries opposite-sign targets). The `run_20260806_183540` session exposed a second, related gap in that same check: **same-sign is not the same as same-target.** Blended joystick input — forward, strafe, and turn all commanded together, which is how people actually drive — can put a same-sign diagonal pair at genuinely different target magnitudes. The raw same-sign filter reported FR−RL RMS 0.105 rad/s on that run, which looked like a real problem. Checked before writing it up: restricting further to samples where the two targets are actually close (`|Δtarget| < 0.1 rad/s`) dropped the number to 0.044, and the *excluded* samples showed target RMS 0.87 against actual RMS 0.86 — the "deviation" was the actual signal correctly following two different commands, not two wheels disagreeing on the same one.

Fixed in `diagonal_rms()`: filters on matched target, not matched sign. Re-ran all six committed analyses (three bench, three ground) under the corrected metric. Result across all three ground sessions, several hours apart: **0.035–0.045 rad/s**, tight and consistent — genuinely no diagonal hardware asymmetry, once measured correctly.

Same underlying lesson as §16.2, worth stating plainly since it's now happened twice with the same shape: a two-wheel-difference metric is only meaningful when the two wheels were actually commanded the same thing. Check *why* two channels differ before treating the number as a fault signal, not just whether they differ.

## 17.3 The ground Kff gap is noisier than the first estimate

The first ground run's three matched-amplitude plateaus (§16.2 update, `run_20260806_152810` report) gave a clean, consistent **+12–15%** PWM-vs-air at matched velocity. Checking the same matched levels across the two later sessions the same day told a messier story: individual plateaus ranged from **+1% to +24%** above the air baseline — not a tight band at all.

Plausible cause: the first run's plateaus were likely closer to a repeated, near-fixed-position pattern; the two later sessions are freer driving covering different physical patches of floor and headings. Floor-texture variation is now a real suspect alongside pure weight-loading, and casual driving simply isn't controlled enough to separate the two.

**This sharpens rather than weakens the case for the structured `staircase` test**: it holds position while sweeping PWM, which isolates the measurement from exactly this location-dependent noise that scattered drive-session plateaus can't avoid. The rough +12–15% number stands as a first estimate and a sanity check, not a value to calibrate against.

## 17.4 Status

Ground loop health: confirmed, three times, hours apart. Ground `Kff`/`Kstat`: still unmeasured by anything rigorous — the drive-session estimates bracket a wide range precisely because they're not the right kind of test. `--mode rotate` removes the floor-space blocker that was in the way of running the real one. Next data expected: ground `staircase`/`plant`, likely in rotate mode.

# Part XVIII — 7 August 2026: Adaptive Control Research, Citation-Checked

Two external research documents proposed moving from static PID+feedforward toward online parameter estimation (RLS), disturbance observers, and full dynamic modeling — motivated by real deployment concerns (this robot's stated goal is industrial deployment under varying payload, floor, and battery conditions). Full response, including a Scite citation check, in `Adaptive_Control_Roadmap.md`; summary here.

**Citation check, since one source was dated 2026 and none had been independently verified:** of four named citations, three checked out cleanly against the actual papers, including the 2026-dated one (Villarreal-López et al., genuinely published mid-2026, not a hallucination). One did not: a claim attributed to "Zhang et al. 2019" (sliding-mode control, fuzzy logic, payload variation) doesn't match what the real 2019 Zhang mecanum paper actually contains (Lyapunov-based adaptive control, no fuzzy logic, addresses slip not payload). Recommendation logged: don't cite that specific claim under that name in the APS or any paper.

**Verdict on the proposal itself:** the core idea — adapt the feedforward model online rather than retuning PID live — is sound and extends work already done here (feedforward already supplies ~90% of control effort; §16.1's Ki fix already established that a wrong feedforward model, not a wrong PID gain, was the real problem). Pushed back on the framing that the current static calibration is "old and trivial": it's the reference baseline any online estimator needs to initialize from and be checked against, not work being superseded. Also pushed back on the illustrative battery-sag scenario specifically for this robot — the LiFePO₄ pack was chosen for its flat discharge curve precisely to blunt that effect (Master_Reference.md §3.5) — while confirming payload/CoM variation, already an open question in B.5, is the real near-term case.

**Direct answer on ML, since it was asked plainly:** no. RLS and Kalman/disturbance-observer methods are classical adaptive control and recursive estimation (Kalman filter: 1960; RLS: same era), not machine learning — no training data, no GPU, deterministic and verifiable in a way a trained model isn't. Where genuine ML would earn its place in this project is perception (Phase 3/4), a separate research thread from wheel control.

**Roadmap logged** (full detail and RLS update equations in the roadmap doc): Stage 0 (finish the static ground calibration already in progress — unchanged) → Stage 1 (2-parameter RLS for Kff/Kstat, on the ESP32, inside the existing 100 Hz loop — lighter than either source's general ESP32-feasibility caution suggests, since our case is 2 parameters not a general UKF) → Stage 2 (disturbance observer for payload/CoM, strongest literature support of anything reviewed — Li & Li 2025's IMC+DOB result, verified above) → Stage 3 (the *already-planned* `robot_localization` EKF, a different filter for a different purpose — pose/velocity fusion on the Pi, not per-motor parameters on the ESP32 — blocked only on IMU procurement, doesn't wait on Stages 1–2) → Stage 4 (gain-scheduled/self-tuning PID, SMC, full online dynamic ID — deferred unless Stages 1–3 prove insufficient on real data).
...
- Decide mounting location — close to the geometric centre. **Reasoning worked through 5 Aug 2026, and the original framing above was imprecise:** tangential acceleration does *not* mix into yaw rate. A gyroscope measures the rigid body's angular velocity ω, which is identical at every point on the body — so gyro placement is irrelevant, and the gyro is the channel that actually solves the Galati heading-drift problem. Only the **accelerometer** is placement-sensitive: off-centre it reads `a_true + α×r + ω×(ω×r)`. At r = 0.433 m (mounting near FR, from l₁=0.403 / d=0.158), ω = 1 rad/s and α = 2 rad/s² give 0.43 m/s² centripetal + 0.87 m/s² tangential against ~0.91 m/s² of real translational acceleration — contamination on the same order as the signal.
  - **Preferred fix is in software, not geometry:** set `imu0_config`'s `ax`/`ay` to `false` in `ekf_params.yaml` and fuse orientation + angular velocity only. Double-integrating a noisy accelerometer for position is counterproductive when wheel encoders already provide translation far more reliably. With `ax`/`ay` off, placement becomes nearly irrelevant — mount wherever is mechanically clean and vibration-quiet. **Not changing the config yet** — no IMU exists to test against, and untested config edits for absent hardware rot.
  - Also note `two_d_mode: true` already discards z / roll / pitch, so the `roll`/`pitch`/`vroll`/`vpitch` `true`s in `imu0_config` are harmless but inert.
- **Two IMUs at diagonal extremes (e.g. near FR and RL) — sequenced, not rejected. Reasoning revised 5 Aug 2026** after a second pass; the first pass undersold the idea and contained an error, both corrected here.

  **The genuine advantage — sensor error becomes directly observable.** Both gyros measure the same rigid-body ω, so differencing them cancels the true signal exactly: `ω₁ − ω₂ = (b₁ − b₂) + (n₁ − n₂)`. What remains is *pure sensor error*, obtained with no external truth reference, no motion-capture rig, no calibration fixture. Under an i.i.d. assumption the per-sensor magnitude follows directly: `σ_single = σ_diff / √2`.

  **What this offers over the standard method.** The textbook approach is Allan variance on a single stationary gyro (IEEE Std 952-1997, updated 952-2020), decomposing noise into angle random walk, rate random walk, bias instability, quantization noise and rate ramp — one sensor, stationary, logging for hours, and cheaper than a second IMU. But a stationary bench test cannot capture vibration-induced noise with four 100 W motors running, g-sensitivity during real acceleration, or thermal drift under operating load. Two differenced gyros work *while the robot is moving*, because real motion cancels. On a mecanum platform where vibration is significant, that gap is real, and characterising it is a defensible methodological contribution in its own right.

  **Diagonal placement specifically adds a scale-factor check.** The two accelerometers at known separation give `a₁ − a₂ = α×Δr + ω×(ω×Δr)`; resolving along Δr yields **ω_z² from accelerometers alone**, independent of the gyros — which catches gyro *scale-factor* error, something gyro-differencing cannot (a common scale error affects both identically and cancels). With the 0.87 m baseline: at ω = 1 rad/s the signal is 0.87 m/s², comfortably above MEMS accelerometer noise; at ω = 0.3 rad/s it is 0.078 m/s², down in the noise once motor vibration is present. Moderate-to-high rotation rates only, and ω² is unsigned so it is magnitude-checking only.

  **Hard limit — state this explicitly if presented.** Two sensors *detect* disagreement but cannot *isolate* which is faulty: `b₁ − b₂` is one equation in two unknowns. Fault isolation requires three (standard redundant-sensor FDI result). This does not affect statistical characterisation across many runs; it does affect runtime fault handling.

  **The strategic reservation that drives the sequencing.** Two IMUs characterise *gyro* drift, but this project's dominant error is *mecanum wheel-slip odometry* drift (Galati: 4.56°/10 m ≈ 0.18 °/s equivalent at 0.4 m/s — comparable in magnitude to consumer MEMS gyro drift, so neither is negligible, but the odometry term is what the aisle-width constraint is sensitive to). The best reference for odometry drift is **SLAM scan-match heading vs. wheel-odometry heading** — already-owned hardware, zero marginal cost, and it measures the quantity the research question is actually about.

  **Correction to the first pass:** it claimed two IMUs "doubles the wiring." That is wrong — I²C is a bus; a second BNO055 shares SDA/SCL/VCC/GND and only needs its ADR pin strapped differently (0x28/0x29, so exactly two fit with no multiplexer). The real objection is different: **I²C is a short-range bus**, and running it ~0.9 m across the chassis past four motors switching tens of amps is a signal-integrity risk (bus capacitance and EMI). Mitigable — 100 kHz instead of 400 kHz, twisted pairs, careful pull-up sizing, possibly a bus extender — but a design problem to solve, not a non-issue.

  **Sequencing:** (1) SLAM + ground test with no IMU — get the scan-match-vs-odometry comparison first, it is free; (2) one IMU, characterised against wheel odometry and SLAM, to establish a baseline; (3) second IMU as a deliberate experiment, so the difference signal has a reference point and its contribution can be stated. `robot_localization` supports `imu0`/`imu1` when that point arrives.
- **Axis count:** 1-axis gyro (ωz) is the true functional requirement; 6-DOF is the practical floor and is genuinely sufficient. Do **not** buy 9-DOF for the magnetometer — indoors, beside four 100 W motors at up to 30 A stall and eventually steel shelving, the mag will likely be unusable, so the BNO055 would run in **IMU mode** (gyro+accel) rather than **NDOF mode**. Absolute heading isn't needed anyway: SLAM supplies the map-frame reference and the IMU's job is short-term drift suppression between scan matches. BNO055 remains the right choice for its in-chip gyro+accel fusion (ESP32 reads a quaternion, runs no AHRS filter), which works fine in IMU mode.
...
- Should per-motor Kff be a single scalar, or a 2-D table over (target velocity × load)? Linear extrapolation of single-point air calibration to load conditions is unverified. **Reframed 7 Aug 2026:** the better answer is probably neither a scalar nor a lookup table — online RLS estimation of `Kff`/`Kstat` (running on the ESP32, updating every control tick from data already being thrown away) tracks load/wear/temperature continuously without needing a table populated in advance. See `Adaptive_Control_Roadmap.md` Stage 1.
...
| 06 Aug 2026  | v2.2        | Added Part XVII: three ground-floor drive sessions the same day, all v3.0 with air-calibrated gains — loop stable across all three (2.7–4.1% RMS error, 0% PWM saturation, zero direction-sign faults, hours apart). Added `--mode rotate` to `nab_pid_logger.py` so the `plant`/`staircase` tests can spin roughly in place instead of driving a straight-line runway, solving a real floor-space blocker. Fixed a second diagonal-check bug in `analyze_bench_log.py`: same-sign targets aren't the same as matched targets, and blended joystick input (forward+strafe+turn together) was producing a false-alarm-shaped number (0.105 rad/s) that was actually correct tracking of two different commands — corrected to a matched-target filter (|Δtarget|<0.1), re-ran all six committed analyses under it, ground diagonal deviation lands at a tight 0.035–0.045 rad/s across all three sessions once measured correctly. Also found the ground Kff gap is noisier than the first estimate suggested — matched-plateau PWM comparisons across the two later sessions ranged +1% to +24% above air (not the clean +12–15% band from the first session), most likely floor-texture/location variability in casual driving — sharpening rather than weakening the case for the structured `staircase` test, which holds position instead of averaging over wherever the robot happened to be. |

| 07 Aug 2026  | v2.3        | Added Part XVIII and `docs/Adaptive_Control_Roadmap.md`: response to two external research documents proposing online parameter estimation (RLS), disturbance observers, and full dynamic modeling as the path beyond static PID+feedforward. Scite-verified all four named citations against the actual papers -- three accurate (including a genuinely real 2026-dated one), one ("Zhang et al. 2019") attached to claims -- SMC, fuzzy logic, payload variation -- that don't match what that paper actually contains. Assessed both documents' engineering recommendations against this project's own measured history rather than accepting them at face value: agreed the feedforward-adaptation approach is sound and extends the existing Ki fix; pushed back on "current model is old and trivial" (it's the reference baseline adaptive methods need, not superseded work) and on the battery-sag motivating scenario specifically for this robot (LiFePO4's flat discharge curve was chosen to blunt exactly that effect); confirmed payload/CoM variation -- already an open question in B.5 -- is the real near-term case. Answered the ML question directly: RLS/Kalman/DOB are classical estimation theory, not ML; genuine ML belongs in perception (Phase 3/4), a separate thread. Logged a 4-stage roadmap sequenced against actual project state, clarifying that the per-motor RLS estimator (ESP32) and the already-planned robot_localization EKF (Pi, blocked only on IMU procurement) are two different filters for two different purposes that shouldn't be conflated. Updated B.5 to point to the new doc. |
