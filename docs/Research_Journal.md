...
- **First-ever Nav2 hardware launch, not yet attempted.** `nav2_slam.launch.py` is written, deployed, built, and never once run. Plan recorded in §17.16: resolve the item above first, then launch in SLAM mode, confirm lifecycle nodes report `active` (not just that the process survives), send a short close first goal (`1.5 m` forward), watch Foxglove throughout, then a return-to-zero goal with a direct check against the physical mark.
