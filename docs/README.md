...
| 5 | [`APS_10Day_Plan.md`](APS_10Day_Plan.md) | **What happens between now and the 23 Sep APS?** Day-by-day plan across all three APS objectives (NAB priority), the precise line between the motion-capability claim the data supports and the symmetric-baseline comparison that Objective 4 still owes, and how the `IoT-Box` and `Fatigue-Detection-` repos feed into this report without being merged into this one. |
...
| [`APS_Seminar_QA_Log_2026-09-21.md`](APS_Seminar_QA_Log_2026-09-21.md) | **Seminar prep Q&A digest**, not a source of record — a conversational walkthrough of the PID/feedforward derivation, which formulas are original vs standard, an informal phone-video check of pose estimation on/off against the dashboard's own `DRIFT` field, and the full multi-cause chain for why no commissioning map has ever been accepted. Defers to the report and journal wherever they disagree. | v1.0, 21 Sep 2026 |
