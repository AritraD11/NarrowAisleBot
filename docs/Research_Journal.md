...
**What this session did not establish.** Whether the scan input improved. Three of four numbers are window-dependent and the historical window is unrecorded. The route to closing it is now cheap and permanent: run `scan_window_sweep.py` on this capture to read flicker at every window length, and from here on record `--seconds` with every `scan_quality.py` figure so no future comparison has this problem. **Stage G's drive still tests exactly what it was built to test — `use_scan_matching: false` and `max_laser_range: 5.0` — and none of that rests on the scan having got better.**
